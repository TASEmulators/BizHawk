...
#define DEBUG(...) NOPRINT(stderr,__VA_ARGS__)
#define DEBUG_PRINT(...) NOPRINT(stderr,__VA_ARGS__)
#define DEBUG_LOAD(...) NOPRINT(stderr,__VA_ARGS__)
#define DEBUG_STORE(...) NOPRINT(stderr,__VA_ARGS__)
#define DEBUG_TRACE(...) DPRINT(stderr,__VA_ARGS__)
...
//zero 07-feb-2012 - alternate computed goto approach 
    ...
    if(PC == 0xB0)
    {
     if(GPR[9] == 0x3D)
     {
      ...
      //}
		...
		//	const int len = GPR[6];
		//	for(int i=0;i<len;i++)
		//		fputc(*(MainRAM.ptr() + (GPR[5]&(2*1024*1024-1)) + i),stderr); //zero 01-apr-2012 - add .ptr
		//}
   }
...
*/
   ...
   //zero 07-feb-2012 - removed __attribute__((unused))
   ...
   //zero 07-feb-2012 - alternate computed goto approach (courtesy of http://abepralle.wordpress.com/2009/01/25/how-not-to-make-a-virtual-machine-label-based-threading/ )
