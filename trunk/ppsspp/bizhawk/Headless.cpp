...
#define BZEXPORT extern "C" __declspec(dllexport)
#define BZAPI __stdcall
...
class CBLogger : public LogListener
{
public:
	void (BZAPI* callback)(char, const char *);
...
WindowsHeadlessHost *headlessHost;
CBLogger *logger;
void *vidbuff;

BZEXPORT int BZAPI init(const char *fn, void (BZAPI* logcallback)(char, const char*))
{
	headlessHost = new HEADLESSHOST_CLASS();
	host = headlessHost;
...
BZEXPORT void BZAPI setvidbuff(void *buff)
{
	vidbuff = buff;
}

BZEXPORT void BZAPI die()
{
	host->ShutdownGL();
	PSP_Shutdown();
...
BZEXPORT void BZAPI advance()
{
	...
	int ret = mipsr4k.RunLoopUntil(nowTicks + frameTicks);

	// ret == 1 means reached ticks limit, so should be exactly equivalent to coreState != CORE_NEXTFRAME

	// If we were rendering, this might be a nice time to do something about it.
	if (coreState == CORE_NEXTFRAME) {
		coreState = CORE_RUNNING;
		if (vidbuff)
			headlessHost->SendBackBuffer(vidbuff);
