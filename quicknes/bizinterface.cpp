...
QN_EXPORT void qn_setup_mappers()
{
	register_optional_mappers();
}

QN_EXPORT Nes_Emu *qn_new()
{
	return new Nes_Emu();
}

QN_EXPORT void qn_delete(Nes_Emu *e)
{
	delete e;
}

QN_EXPORT const char *qn_loadines(Nes_Emu *e, const void *data, int length)
{
...
QN_EXPORT const char *qn_set_sample_rate(Nes_Emu *e, int rate)
{
...
QN_EXPORT const char *qn_emulate_frame(Nes_Emu *e, int pad1, int pad2)
{
	return e->emulate_frame(pad1, pad2);
}

QN_EXPORT void qn_blit(Nes_Emu *e, int32_t *dest, const int32_t *colors, int cropleft, int croptop, int cropright, int cropbottom)
{
...
QN_EXPORT const Nes_Emu::rgb_t *qn_get_default_colors()
{
	return Nes_Emu::nes_colors;
}

QN_EXPORT int qn_get_joypad_read_count(Nes_Emu *e)
{
	return e->frame().joypad_read_count;
}

QN_EXPORT void qn_get_audio_info(Nes_Emu *e, int *sample_count, int *chan_count)
{
	if (sample_count)
...
QN_EXPORT int qn_read_audio(Nes_Emu *e, short *dest, int max_samples)
{
	return e->read_samples(dest, max_samples);
}

QN_EXPORT void qn_reset(Nes_Emu *e, int hard)
{
	e->reset(hard);
}

QN_EXPORT const char *qn_state_size(Nes_Emu *e, int *size)
{
	Sim_Writer w;
...
QN_EXPORT const char *qn_state_save(Nes_Emu *e, void *dest, int size)
{
...
QN_EXPORT const char *qn_state_load(Nes_Emu *e, const void *src, int size)
{
...
QN_EXPORT int qn_has_battery_ram(Nes_Emu *e)
{
	return e->has_battery_ram();
}

QN_EXPORT const char *qn_battery_ram_size(Nes_Emu *e, int *size)
{
	Sim_Writer w;
	Auto_File_Writer a(w);
...
QN_EXPORT const char *qn_battery_ram_save(Nes_Emu *e, void *dest, int size)
{
	Mem_Writer w(dest, size, 0);
	Auto_File_Writer a(w);
	const char *ret = e->save_battery_ram(a);
...
QN_EXPORT const char *qn_battery_ram_load(Nes_Emu *e, const void *src, int size)
{
	Mem_File_Reader r(src, size);
	Auto_File_Reader a(r);
...
QN_EXPORT const char *qn_battery_ram_clear(Nes_Emu *e)
{
	int size = 0;
...
QN_EXPORT void qn_set_sprite_limit(Nes_Emu *e, int n)
{
	e->set_sprite_mode((Nes_Emu::sprite_mode_t)n);
}

QN_EXPORT int qn_get_memory_area(Nes_Emu *e, int which, const void **data, int *size, int *writable, const char **name)
{
...
QN_EXPORT unsigned char qn_peek_prgbus(Nes_Emu *e, int addr)
{
	return e->peek_prg(addr & 0xffff);
}

QN_EXPORT void qn_poke_prgbus(Nes_Emu *e, int addr, unsigned char val)
{
	e->poke_prg(addr & 0xffff, val);
}

QN_EXPORT void qn_get_cpuregs(Nes_Emu *e, unsigned int *dest)
{
	e->get_regs(dest);
}

QN_EXPORT const char *qn_get_mapper(Nes_Emu *e, int *number)
{
...
QN_EXPORT byte qn_get_reg2000(Nes_Emu *e)
{
	return e->get_ppu2000();
}

QN_EXPORT byte *qn_get_palmem(Nes_Emu *e)
{
	return e->pal_mem();
}

QN_EXPORT byte *qn_get_oammem(Nes_Emu *e)
{
	return e->oam_mem();
}

QN_EXPORT byte qn_peek_ppu(Nes_Emu *e, int addr)
{
	return e->peek_ppu(addr);
}

QN_EXPORT void qn_peek_ppubus(Nes_Emu *e, byte *dest)
{
	for (int i = 0; i < 0x3000; i++)
		dest[i] = e->peek_ppu(i);
}

QN_EXPORT void qn_set_tracecb(Nes_Emu *e, void (*cb)(unsigned int *dest))
{
	e->set_tracecb(cb);
}
