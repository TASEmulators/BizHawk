...
namespace BizHawk.Emulation.Consoles.Calculator
{
    public class TI83 : IEmulator
    {
                ...
                case 3: //PORT_STATUS
                    {
                        ...
                        ret = (byte)((Controller.IsPressed("ON") ? maskOn : 8) | (LinkActive ? 0 : 2));
                        return ret;
                    }
        ...
        public class Link
        {
            ...
            public void SendFileToCalc(FileStream FS)
            {
                FS.Seek(53, SeekOrigin.Begin);
                int Size = FS.ReadByte() + FS.ReadByte() * 256;

                if (FS.Length != Size + 57)
                    throw new FormatException("Invalid calculator file.");

                byte[] Header = new byte[13];
                FS.Read(Header, 0, 13);

                FileData = new byte[Size - 13];
                FS.Read(FileData, 0, Size - 13);
                ...
                CurrentStatus = Status.PrepareReceive;
                NextStep = new Action(ReceiveReqAck);
            ...
            private void ReceiveReqAck()
            {
                ...
                BytesToSend = 8;
                CurrentStatus = Status.PrepareSend;
                NextStep = new Action(SendVariableData);
            }
            ...
            private void SendVariableData()
            {
                ...
                else
                {
                    ...
                    ushort Checksum = 0;
                    for (int n = 2; n < FileData.Length; n++)
                        Checksum += FileData[n];
                    ...
                    CurrentStatus = Status.PrepareReceive;
                    NextStep = new Action(ReceiveDataAck);
                    Parent.LinkActive = true;
                }
            }
            ...
            private void ReceiveDataAck()
            {
                ...
                BytesToSend = 4;
                CurrentStatus = Status.PrepareSend;
                NextStep = new Action(EndTransmission);
            }
            ...
            private void OutOfMemory()
            {
                ...
                BytesToSend = 3;
                CurrentStatus = Status.PrepareSend;
                NextStep = new Action(EndOutOfMemory);
            }
