...
namespace BizHawk.Client.EtoHawk
{
	public class OTK_GamePad
	{
		...
		{
			foreach (JoystickAxis pi in Enum.GetValues(typeof(JoystickAxis)))
		...
		void InitializeCallbacks()
		{
			...
			AddItem("X+", () => state.GetAxis(JoystickAxis.Axis0) >= dzp);
			AddItem("X-", () => state.GetAxis(JoystickAxis.Axis0) <= dzn);
			AddItem("Y+", () => state.GetAxis(JoystickAxis.Axis1) >= dzp);
			AddItem("Y-", () => state.GetAxis(JoystickAxis.Axis1) <= dzn);
			AddItem("Z+", () => state.GetAxis(JoystickAxis.Axis2) >= dzp);
			AddItem("Z-", () => state.GetAxis(JoystickAxis.Axis2) <= dzn);

			// Enjoy our delicious sliders. They're smaller than regular burgers but cost more.

			int jb = 1;
			foreach (JoystickButton enval in Enum.GetValues(typeof(JoystickButton)))
			{
				AddItem(string.Format("B{0}", jb), () => state.GetButton(enval)==ButtonState.Pressed);
				jb++;
			}
