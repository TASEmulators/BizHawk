...
void V810::CheckBreakpoints(void (*callback)(int type, uint32 address, uint32 value, unsigned int len), uint16 MDFN_FASTCALL (*peek16)(const v810_timestamp_t, uint32), uint32 MDFN_FASTCALL (*peek32)(const v810_timestamp_t, uint32))
{
	...
	case LD_B:
		callback(BPOINT_READ, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFF, 0, 1);
		break;
	case LD_H:
		callback(BPOINT_READ, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFE, 0, 2);
		break;
	case LD_W:
		callback(BPOINT_READ, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFC, 0, 4);
		break;

	case ST_B:
		callback(BPOINT_WRITE, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFF, P_REG[(tmpop >> 5) & 0x1F] & 0x00FF, 1);
		break;
	case ST_H:
		callback(BPOINT_WRITE, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFE, P_REG[(tmpop >> 5) & 0x1F] & 0xFFFF, 2);
		break;
	case ST_W:
		callback(BPOINT_WRITE, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFC, P_REG[(tmpop >> 5) & 0x1F], 4);
		break;

	case IN_B:
		callback(BPOINT_IO_READ, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFF, 0, 1);
		break;
	case IN_H:
		callback(BPOINT_IO_READ, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFE, 0, 2);
		break;
	case IN_W:
		callback(BPOINT_IO_READ, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFC, 0, 4);
		break;

	case OUT_B:
		callback(BPOINT_IO_WRITE, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFF, P_REG[(tmpop >> 5) & 0x1F] & 0xFF, 1);
		break;
	case OUT_H:
		callback(BPOINT_IO_WRITE, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFE, P_REG[(tmpop >> 5) & 0x1F] & 0xFFFF, 2);
		break;
	case OUT_W:
		callback(BPOINT_IO_WRITE, (sign_16(tmpop_high) + P_REG[tmpop & 0x1F]) & 0xFFFFFFFC, P_REG[(tmpop >> 5) & 0x1F], 4);
		break;
	}
}
#endif
...
*/
#define RB_CPUHOOK_DBG(n)                  \
	{                                      \
		if (CPUHook)                       \
		{                                  \
			v810_timestamp = timestamp_rl; \
			CPUHook(timestamp_rl, n);      \
		}                                  \
	}
