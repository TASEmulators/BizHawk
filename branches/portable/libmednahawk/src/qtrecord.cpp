...
{
 //zero 07-feb-2012 - remove dynamic stack buffer
			  ...
			  const int16 *SoundBuf, const int32 SoundBufSize)
{
 ...
 {
  //zero 07-feb-2012 - remove dynamic stack buffer
   int sizeof_abuf = SoundBufSize * SoundChan*2;
  int16  *abuf = new int16[sizeof_abuf/2];

  for(int i = 0; i < SoundBufSize * SoundChan; i++)
   MDFN_en16msb((uint8 *)&abuf[i], SoundBuf[i]);

  qtfile.write(abuf, sizeof_abuf);
  delete[] abuf;
 }
