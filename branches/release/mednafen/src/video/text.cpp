...
static FontDescriptor_t FontDescriptors[_MDFN_FONT_COUNT] =
{
 ...
 { 5, 7, 	-1,			sizeof(FontData5x7[0]),		&FontData5x7[0].data[0] },  //zero 29-oct-2012 - what are these parents doing here FOR RYPHECHA
 { 4, 5,	-1,			sizeof(FontData4x5[0]),		&FontData4x5[0].data[0] }, //zero 29-oct-2012 - what are these parents doing here FOR RYPHECHA
...
{
  //zero 07-feb-2012 - remove dynamic stack buffers...

 uint32 slen;
 uint32 pixwidth;
 uint32 max_glyph_len = strlen((char *)msg);
 const uint8 **glyph_ptrs = new const uint8*[max_glyph_len];
 uint8 *glyph_width = new uint8[max_glyph_len];
 uint8 *glyph_ov_width= new uint8[max_glyph_len];

 const UTF8 *src_begin = (UTF8 *)msg;
 UTF32* utf32_buf = new UTF32[max_glyph_len];
...
uint32 DrawTextTrans(uint32 *dest, int pitch, uint32 width, const UTF8 *msg, uint32 fgcolor, int centered, uint32 which_font)
{
 ...
 uint32 ret = (DoRealDraw(dest, pitch, width, fgcolor, centered, slen, pixwidth, FontDescriptors[which_font].glyph_height, glyph_ptrs, glyph_width, glyph_ov_width));
