...
");
		...
		{
			SublimeCompletions sc = new SublimeCompletions();

			foreach (var f in this.OrderBy(lf => lf.Library).ThenBy(lf => lf.Name))
			{
				SublimeCompletions.Completion completion = new SublimeCompletions.Completion
				{
				...
				if (f.ParameterList.Any())
				{
					...
					List<ParameterInfo> parameters = f.Method.GetParameters()
						.ToList();
