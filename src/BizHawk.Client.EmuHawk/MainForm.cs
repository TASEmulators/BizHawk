...
//				Util.DebugWriteLine($"  Selecting display size {lastComputedSize}");
		...
		public void LoadState(BinaryReader coreData, IVideoProvider stateScreenshotProvider, string userFriendlyStateName = null)
		{
		...
		public bool LoadState(string path, string userFriendlyStateName, bool suppressOSD = false) // Move to client.common
		{
			...
			RA?.OnLoadState(path);
			LoadStateCommon(suppressOSD ? null : userFriendlyStateName);
