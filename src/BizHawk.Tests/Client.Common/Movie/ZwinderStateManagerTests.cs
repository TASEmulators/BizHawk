...
namespace BizHawk.Tests.Client.Common.Movie
{
	...
	public class ZwinderStateManagerTests
	{
		...
		{
			ZwinderStateManager zw = new ZwinderStateManager(new ZwinderStateManagerSettings
			{
				CurrentBufferSize = 1,
		...
		public void SaveCreateRoundTrip()
		{
			...
			MemoryStream rms = new MemoryStream(buff, false);

			ZwinderStateManager zw2 = ZwinderStateManager.Create(new BinaryReader(rms), zw.Settings, f => false);
		...
		{
			using ZwinderBuffer zb = new ZwinderBuffer(new RewindConfig
			{
				BufferSize = 1,
				TargetFrameLength = 1
			});
		...
		public void SaveCreateBufferRoundTrip()
		{
			...
			ZwinderBuffer buff = new ZwinderBuffer(config);
			StateSource ss = new StateSource { PaddingData = new byte[500] };
			for (int frame = 0; frame < 2090; frame++)
			{
				ss.Frame = frame;
			...
			ms.Position = 0;
			ZwinderBuffer buff2 = ZwinderBuffer.Create(new BinaryReader(ms), config);
		...
		{
			StateSource ss = new StateSource { PaddingData = new byte[1000] };
			ZwinderStateManager zw = new ZwinderStateManager(new ZwinderStateManagerSettings
			{
				CurrentBufferSize = 1,
				CurrentTargetFrameLength = 10000,
			...
			int actual = StateSource.GetFrameNumberInState(data);
		...
		public void HasState_Correct_WhenReservedGreaterThanCurrent()
		{
			...
			// Act
			bool actual = zw.HasState(futureReservedFrame);
		...
		public void HasState_Correct_WhenCurrentIsLast()
		{
			...
			// Act
			bool actual = zw.HasState(totalCurrentFrames - expectedFrameGap);
		...
		public void Count_NoReserved()
		{
			...
			// Assert
			int expected = (totalCurrentFrames / expectedFrameGap) + 1;
		...
		public void Count_WithReserved()
		{
			...
			// Assert
			int expected = (totalCurrentFrames / expectedFrameGap) + 2;
			Assert.AreEqual(expected, actual);
		}
		...
		public void StateCache()
		{
			...
			System.Collections.Generic.List<int> allStates = zw.AllStates()
				.Select(s => s.Frame)
				.ToList();

			for (int i = 0; i < 10000; i++)
			{
				bool actual = zw.HasState(i);
				bool expected = allStates.Contains(i);
				Assert.AreEqual(expected, actual);
			}
		}
		...
		public void WhatIfTheHeadStateWrapsAround()
		{
			...
			using ZwinderBuffer zw = new ZwinderBuffer(new RewindConfig
			{
				BufferSize = 1,
				TargetFrameLength = 1
			});
