...
namespace BizHawk.Common
{
	public class DescribableEnumConverter : EnumConverter
	{
		...
		{
			string valueStr = value?.ToString() ?? throw new ArgumentNullException(paramName: nameof(value));
			return Enum.Parse(
				enumType,
		...
		{
			string fieldName = Enum.GetName(enumType, value ?? throw new ArgumentNullException(paramName: nameof(value)));
			if (fieldName != null)
			{
