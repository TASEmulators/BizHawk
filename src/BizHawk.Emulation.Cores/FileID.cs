...
namespace BizHawk.Emulation.Cores
{
	...
	public class FileID
	{
		...
		public FileIDResults Identify(IdentifyParams p)
		{
			...
			if(job.Extension != null)
			{
				...
				if (ExtensionHandlers.TryGetValue(ext, out var handler))
				{
					...
					{
						FileIDResult fidr = new(handler.DefaultForExtension, 5);
						ret.Add(fidr);
					}
				}
			}
		...
		{
			DiscSystem.DiscIdentifier discIdentifier = new(job.Disc);
			...
			{
				DiscSystem.DiscType.SegaSaturn => new FileIDResults(new FileIDResult(FileIDType.Saturn, 100)),
				DiscSystem.DiscType.SonyPSP => new FileIDResults(new FileIDResult(FileIDType.PSP, 100)),
				DiscSystem.DiscType.SonyPSX => new FileIDResults(new FileIDResult(FileIDType.PSX, 100)),
				DiscSystem.DiscType.MegaCD => new FileIDResults(new FileIDResult(FileIDType.MegaCD, 100)),
				DiscSystem.DiscType.TurboCD => new FileIDResults(new FileIDResult(FileIDType.TurboCD, 5)),
				_ => new FileIDResults(new FileIDResult()),
			};
		}
		...
		{
			public static readonly SimpleMagicRecord INES = new() { Offset = 0, Key = "NES" };
			public static readonly SimpleMagicRecord UNIF = new() { Offset = 0, Key = "UNIF" };
			public static SimpleMagicRecord NSF = new() { Offset = 0, Key = "NESM\x1A" }; 

			public static readonly SimpleMagicRecord FDS_HEADERLESS = new() { Offset = 0, Key = "\x01*NINTENDO-HVC*" };
			public static readonly SimpleMagicRecord FDS_HEADER = new() { Offset = 0, Key = "FDS\x1A" };
			...
			public static readonly SimpleMagicRecord GBA = new() {  Offset = 4, Length = 16, Key = "" };
			public static readonly SimpleMagicRecord NDS = new() { Offset = 0xC0, Length = 16, Key = "" };

			public static readonly SimpleMagicRecord GB = new() {  Offset=0x104, Length = 16, Key = "\xCE\xED\x66\x66\xCC\x0D\x00\x0B\x03\x73\x00\x83\x00\x0C\x00\x0D\x00\x08\x11\x1F\x88\x89\x00\x0E\xDC\xCC\x6E\xE6\xDD\xDD\xD9\x99\xBB\xBB\x67\x63\x6E\x0E\xEC\xCC\xDD\xDC\x99\x9F\xBB\xB9\x33\x3E" };

			public static readonly SimpleMagicRecord S32X = new() { Offset = 0x100, Key = "SEGA 32X" };

			public static readonly SimpleMagicRecord SEGAGENESIS = new() { Offset = 0x100, Key = "SEGA GENESIS" };
			public static readonly SimpleMagicRecord SEGAMEGADRIVE = new() { Offset = 0x100, Key = "SEGA MEGA DRIVE" };
			public static readonly SimpleMagicRecord SEGASATURN = new() { Offset = 0, Key = "SEGA SEGASATURN" };
			public static readonly SimpleMagicRecord SEGADISCSYSTEM = new() { Offset = 0, Key = "SEGADISCSYSTEM" };

			public static readonly SimpleMagicRecord PSX = new() { Offset = 0x24E0, Key = "  Licensed  by          Sony Computer Entertainment" }; //there might be other ideas for checking in mednafen sources, if we need them
			public static readonly SimpleMagicRecord PSX_EXE = new() { Key = "PS-X EXE\0" };
			public static readonly SimpleMagicRecord PSP = new() { Offset = 0x8000, Key = "\x01CD001\x01\0x00PSP GAME" };
			public static readonly SimpleMagicRecord PSF = new() { Offset = 0, Key = "PSF\x1" };

			//https://sites.google.com/site/atari7800wiki/a78-header
			public static readonly SimpleMagicRecord A78 = new() { Offset = 0, Key = "\x01ATARI7800" };

			//could be at various offsets?
			public static SimpleMagicRecord TMR_SEGA = new() { Offset = 0x7FF0, Key = "TMR SEGA" };

			public static readonly SimpleMagicRecord SBI = new() { Key = "SBI\0" };
			public static readonly SimpleMagicRecord M3U = new() { Key = "#EXTM3U" }; //note: M3U may not have this. EXTM3U only has it. We'll still catch it by extension though.

			public static readonly SimpleMagicRecord ECM = new() { Key = "ECM\0" };
			public static readonly SimpleMagicRecord FLAC = new() { Key = "fLaC" };
			public static readonly SimpleMagicRecord MPC = new() { Key = "MP+", ExtraCheck = (s) => { s.Position += 3; return s.ReadByte() >= 7; } };
			public static readonly SimpleMagicRecord APE = new() { Key = "MAC " };
		...
		/// </summary>
		private static readonly Dictionary<string, ExtensionInfo> ExtensionHandlers = new()
		{
		...
		private static FileIDResult Test_INES(IdentifyJob job)
		{
			...
			FileIDResult ret = new(FileIDType.INES, 100);
		...
		private static FileIDResult Test_UNIF(IdentifyJob job)
		{
			...
			FileIDResult ret = new(FileIDType.UNIF, 100);
		...
		private static FileIDResult Test_GB_GBC(IdentifyJob job)
		{
			...
			FileIDResult ret = new(FileIDType.GB, 100);
		...
		private static FileIDResult Test_N64(IdentifyJob job)
		{
			...
			FileIDResult ret = new(FileIDType.N64, 5);
		...
		private static FileIDResult Test_BIN_ISO(IdentifyJob job)
		{
			...
			{
				FileIDResult ret = new(FileIDType.PSX, 95);
