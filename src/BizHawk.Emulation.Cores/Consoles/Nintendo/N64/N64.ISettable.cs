...
namespace BizHawk.Emulation.Cores.Nintendo.N64
{
	public partial class N64 : ISettable<N64Settings, N64SyncSettings>
	{
		...
		private void SetControllerButtons(N64SyncSettings syncSettings)
		{
			...
			{
				def.BoolButtons.AddRange(new[] { $"P{player} DPad U", $"P{player} DPad D", $"P{player} DPad L", $"P{player} DPad R", $"P{player} Start", $"P{player} Z", $"P{player} B", $"P{player} A", $"P{player} C Up", $"P{player} C Down", $"P{player} C Left", $"P{player} C Right", $"P{player} L", $"P{player} R" });
				def.AddXYPair(
					$"P{player} {{0}} Axis",
