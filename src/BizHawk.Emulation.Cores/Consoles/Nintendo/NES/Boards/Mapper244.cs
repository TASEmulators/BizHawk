...
namespace BizHawk.Emulation.Cores.Nintendo.NES
{
	internal sealed class Mapper244 : NesBoardBase
	{
		...
		private readonly List<List<byte>> prg_perm = new()
		{
		...
		private readonly List<List<byte>> chr_perm = new()
		{
		...
		public override byte ReadPrg(int addr) => Rom[(_prgRegister * 0x8000) + (addr & 0x7FFF)];
