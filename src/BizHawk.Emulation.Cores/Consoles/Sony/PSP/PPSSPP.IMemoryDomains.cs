...
namespace BizHawk.Emulation.Cores.Consoles.Sony.PSP
{
	public partial class PPSSPP
	{
		...
		{
			void WireDomain(LibPPSSPP.MemoryRegion region, MemoryDomainIntPtr domain)
			{
				_core.PPSSPP_GetMemoryRegion(_context, region, out var ptr, out var size);
				domain.Data = ptr;
				domain.SetSize(size);
			}

			WireDomain(LibPPSSPP.MemoryRegion.FCRAM, _fcram);
			WireDomain(LibPPSSPP.MemoryRegion.VRAM, _vram);
			WireDomain(LibPPSSPP.MemoryRegion.DSP, _dspRam);
			WireDomain(LibPPSSPP.MemoryRegion.N3DS, _n3dsExRam);
		}

		private class EncoreMMU : MemoryDomain
		{
			private const uint PPSSPP_PAGE_SIZE = 0x1000;
			private const uint PPSSPP_PAGE_MASK = PPSSPP_PAGE_SIZE - 1;
			...
			{
				var pagePointer = _core.PPSSPP_GetPagePointer(_context, addr);
				return pagePointer == IntPtr.Zero ? [ ] : Util.UnsafeSpanFromPointer(pagePointer, (int)(PPSSPP_PAGE_SIZE - (addr & PPSSPP_PAGE_MASK)));
			}
			...
			public override ushort PeekUshort(long addr, bool bigEndian)
			{
				...
				if ((addr & PPSSPP_PAGE_MASK) > PPSSPP_PAGE_MASK - 1)
				{
			...
			{
				// if we cross a page boundary, we need to read multiple pages
				if ((addr & PPSSPP_PAGE_MASK) > PPSSPP_PAGE_MASK - 3)
				{
			...
			private void BulkPeekByte(uint startAddr, Span<byte> values)
			{
				while (!values.IsEmpty)
				{
					...
					var numBytes = Math.Min(values.Length, (int)(PPSSPP_PAGE_SIZE - (startAddr & PPSSPP_PAGE_MASK)));
					if (page.IsEmpty)
					{
