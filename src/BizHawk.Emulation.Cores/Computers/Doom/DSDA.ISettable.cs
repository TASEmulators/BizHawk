...
{
	public partial class DSDA : ISettable<object, DSDA.DSDASyncSettings>
	{
		...
		private void CreateArguments(DSDASyncSettings settings)
		{
			...
			if (settings.FastMemory != LibDSDA.FASTMEM_AUTO)
			{
			...
			{
				LibDSDA.ControllerType type = port == 0
		...
		private DSDASyncSettings _syncSettings;
		public DSDASyncSettings GetSyncSettings()
			=> _syncSettings.Clone();

		public PutSettingsDirtyBits PutSyncSettings(DSDASyncSettings o)
		{
			var ret = DSDASyncSettings.NeedsReboot(_syncSettings, o);
			_syncSettings = o;
			return ret ? PutSettingsDirtyBits.RebootCore : PutSettingsDirtyBits.None;
		}

		[CoreSettings]
		public class DSDASyncSettings
		{
			...
			[Range(LibDSDA.FASTMEM_AUTO, 512)]
			[DefaultValue(LibDSDA.FASTMEM_AUTO)]
			[TypeConverter(typeof(ConstrainedIntConverter))]
			public int FastMemory { get; set; }

			[DisplayName("Controller port 1")]
			[Description("")]
			[DefaultValue(LibDSDA.ControllerType.Mouse)]
			[TypeConverter(typeof(DescribableEnumConverter))]
			public LibDSDA.ControllerType ControllerPort1 { get; set; }

			[DisplayName("Controller port 2")]
			[Description("")]
			[DefaultValue(LibDSDA.ControllerType.Joystick)]
			[TypeConverter(typeof(DescribableEnumConverter))]
			public LibDSDA.ControllerType ControllerPort2 { get; set; }
			...
			public DSDASyncSettings()
				=> SettingsUtil.SetDefaultValues(this);

			public DSDASyncSettings Clone()
				=> (DSDASyncSettings)MemberwiseClone();

			public static bool NeedsReboot(DSDASyncSettings x, DSDASyncSettings y)
