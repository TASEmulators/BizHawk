...
namespace BizHawk.Emulation.Cores.Waterbox
{
	...
	public unsafe class WaterboxMemoryDomainSizedFuncs : WaterboxMemoryDomain
	{
		...
		private readonly MemoryDomainSizedAccessStub _read64 = null;
		private readonly MemoryDomainSizedAccessStub _write64 = null;
			...
			: base(m, monitor)
		{
			...
			if (functionPointers[0] != IntPtr.Zero) _read8 = MemoryDomainSizedAccessStub.Create(functionPointers[0], monitor);
			if (functionPointers[1] != IntPtr.Zero) _write8 = MemoryDomainSizedAccessStub.Create(functionPointers[1], monitor);

			if (functionPointers[2] != IntPtr.Zero) _read16 = MemoryDomainSizedAccessStub.Create(functionPointers[2], monitor);
			if (functionPointers[3] != IntPtr.Zero) _write16 = MemoryDomainSizedAccessStub.Create(functionPointers[3], monitor);

			if (functionPointers[4] != IntPtr.Zero) _read32 = MemoryDomainSizedAccessStub.Create(functionPointers[4], monitor);
			if (functionPointers[5] != IntPtr.Zero) _write32 = MemoryDomainSizedAccessStub.Create(functionPointers[5], monitor);

			if (functionPointers[6] != IntPtr.Zero) _read64 = MemoryDomainSizedAccessStub.Create(functionPointers[6], monitor);
			if (functionPointers[7] != IntPtr.Zero) _write64 = MemoryDomainSizedAccessStub.Create(functionPointers[7], monitor);
		}

		public override byte PeekByte(long addr)
		{
			if (_read8 == null) throw new InvalidOperationException($"{Name} does not support 8-bit reads.");
		...
		public override void PokeByte(long addr, byte val)
		{
			if (_write8 == null) throw new InvalidOperationException($"{Name} does not support 8-bit writes.");
			if ((ulong)addr >= (ulong)Size || addr < 0) throw new ArgumentOutOfRangeException(nameof(addr));
		...
		{
			if (_read16 == null) throw new InvalidOperationException($"{Name} does not support 16-bit reads.");
		...
		{
			if (_write16 == null) throw new InvalidOperationException($"{Name} does not support 16-bit writes.");
			if ((ulong)addr + 1 >= (ulong)Size || addr < 0) throw new ArgumentOutOfRangeException(nameof(addr));
		...
		{
			if (_read32 == null) throw new InvalidOperationException($"{Name} does not support 32-bit reads.");
		...
		{
			if (_write32 == null) throw new InvalidOperationException($"{Name} does not support 32-bit writes.");
			if ((ulong)addr + 3 >= (ulong)Size || addr < 0) throw new ArgumentOutOfRangeException(nameof(addr));
