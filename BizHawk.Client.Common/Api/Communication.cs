...
namespace BizHawk.Client.Common
{
	...
	{
		public static Func<IVideoProvider, BitmapBuffer> RenderScreenshotCallback { private get; set; }
		...
		public class MemoryMappedFiles
		{
			...
			public int WriteStringToFile(string filename, string outputString) => WriteToFile(filename, Encoding.ASCII.GetBytes(outputString));
		...
		class ScreenShot
		{
			...
			{
				if (RenderScreenshotCallback == null) throw new Exception();
				return RenderScreenshotCallback(_currentVideoProvider ??= Global.Emulator.AsVideoProviderOrDefault());
			}
