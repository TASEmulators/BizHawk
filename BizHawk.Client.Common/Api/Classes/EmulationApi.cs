...
namespace BizHawk.Client.Common
{
	...
	public sealed class EmulationApi : IEmulation
	{
		[OptionalService]
		...
		public string BoardName => BoardInfo?.BoardName ?? string.Empty;

		public string DisplayType => (RegionableCore?.Region)?.ToString() ?? string.Empty;

		/// <summary>Using this property to get a reference to <see cref="Global.Config">Global.Config</see> is a terrible, horrible, no good, very bad idea. That's why it's not in the <see cref="IEmulation">interface</see>.</summary>
		...
		public bool IsLagged
		{
			get
			{
				...
				_logCallback($"Can not get lag information, {Emulator.Attributes().CoreName} does not implement {nameof(IInputPollable)}");
				return false;
			}
			set
			{
				if (InputPollableCore != null) InputPollableCore.IsLagFrame = value;
				else _logCallback($"Can not set lag information, {Emulator.Attributes().CoreName} does not implement {nameof(IInputPollable)}");
			}
		}

		public bool IsVSyncEnabled
		{
			get => Global.Config.VSync;
			set => Global.Config.VSync = value;
		}
		...
		public int LagCount
		{
			...
			set
			{
				if (InputPollableCore != null) InputPollableCore.LagCount = value;
				else _logCallback($"Can not set lag information, {Emulator.Attributes().CoreName} does not implement {nameof(IInputPollable)}");
			}
		}
		...
		{
			get => Global.Config.ClockThrottle;
			set => Global.Config.ClockThrottle = value;
		}

		public bool MinimizeFrameskip
		{
			get => Global.Config.AutoMinimizeSkipping;
			set => Global.Config.AutoMinimizeSkipping = value;
		}

		public string SystemID => Global.Game.System;
		...
		public long TotalExecutedCycles
		{
			get
			{
				...
				_logCallback($"Error: {Emulator.Attributes().CoreName} does not yet implement {nameof(IDebuggable.TotalExecutedCycles)}()");
				return default;
			}
		}
		...
		public object Disassemble(uint pc, string name)
		{
			...
			_logCallback($"Error: {Emulator.Attributes().CoreName} does not yet implement {nameof(IDisassemblable.Disassemble)}()");
			return null;
		}
		...
		public ulong? GetRegister(string name)
		{
			...
			_logCallback($"Error: {Emulator.Attributes().CoreName} does not yet implement {nameof(IDebuggable.GetCpuFlagsAndRegisters)}()");
			return null;
		}

		public IDictionary<string, ulong> GetRegisters()
		{
			try
			{
				if (DebuggableCore != null)
		...
		public void SetRegister(string register, int value)
		{
			...
			_logCallback($"Error: {Emulator.Attributes().CoreName} does not yet implement {nameof(IDebuggable.SetCpuRegister)}()");
		}
