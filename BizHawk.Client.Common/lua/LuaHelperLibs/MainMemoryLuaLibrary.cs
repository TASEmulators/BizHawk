...
namespace BizHawk.Client.Common
{
	...
	public sealed class MainMemoryLuaLibrary : DelegatingLuaLibrary
	{
		...
		public string GetName() => Domain.Name;
		...
		public uint ReadByte(int addr) => APIs.MemoryAccess.ReadByte(addr, Domain.Name);
		...
		public void WriteByte(int addr, uint value) => APIs.MemoryAccess.WriteByte(addr, value, Domain.Name);
		...
		public LuaTable ReadByteRange(int addr, int length) => APIs.MemoryAccess.ReadByteRange(addr, length, Domain.Name).ToLuaTable(Lua);
		...
		{
#if true
			foreach (var addr in memoryblock.Keys) APIs.MemoryAccess.WriteByte(LuaInt(addr), (uint) memoryblock[addr], Domain.Name);
#else
			var d = Domain;
			if (d.CanPoke())
			{
		...
		public float ReadFloat(int addr, bool bigendian)
		{
			...
			return APIs.MemoryAccess.ReadFloat(addr, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = bigendian;
			APIs.MemoryAccess.WriteFloat(addr, value, Domain.Name);
		}
		...
		public int ReadS8(int addr) => APIs.MemoryAccess.ReadS8(addr, Domain.Name);

		[LuaMethodExample("mainmemory.write_s8( 0x100, 1000 );")]
		[LuaMethod("write_s8", "write signed byte")]
		public void WriteS8(int addr, uint value) => APIs.MemoryAccess.WriteS8(addr, unchecked((int) value), Domain.Name);

		[LuaMethodExample("local uimairea = mainmemory.read_u8( 0x100 );")]
		[LuaMethod("read_u8", "read unsigned byte")]
		public uint ReadU8(int addr) => APIs.MemoryAccess.ReadU8(addr, Domain.Name);

		[LuaMethodExample("mainmemory.write_u8( 0x100, 1000 );")]
		[LuaMethod("write_u8", "write unsigned byte")]
		public void WriteU8(int addr, uint value) => APIs.MemoryAccess.WriteU8(addr, value, Domain.Name);
		...
		public int ReadS16Little(int addr)
		{
			...
			return APIs.MemoryAccess.ReadS16(addr, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			APIs.MemoryAccess.WriteS16(addr, value, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			return APIs.MemoryAccess.ReadU16(addr, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			APIs.MemoryAccess.WriteU16(addr, value, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			return APIs.MemoryAccess.ReadS24(addr, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			APIs.MemoryAccess.WriteS24(addr, value, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			return APIs.MemoryAccess.ReadU24(addr, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			APIs.MemoryAccess.WriteU24(addr, value, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			return APIs.MemoryAccess.ReadS32(addr, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			APIs.MemoryAccess.WriteS32(addr, value, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			return APIs.MemoryAccess.ReadU32(addr, Domain.Name);
		}
		...
		{
			APIs.MemoryAccess.BigEndian = false;
			APIs.MemoryAccess.WriteU32(addr, value, Domain.Name);
		}
