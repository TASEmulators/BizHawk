...
namespace SevenZip
{
...
#endif
	{
		...
		public void ExtractFile(string fileName, Stream stream)
		{
			...
			if (index == -1)
			{
				if (!ThrowException(null, new ArgumentOutOfRangeException("fileName",
		...
		public void ExtractFile(int index, Stream stream)
		{
			...
			{
				if (!ThrowException(null, new ArgumentOutOfRangeException("index",
					"The specified index is greater than the archive files count.")))
				{
					return;
				}
			}
		...
		public void ExtractFiles(string directory, params int[] indexes)
		{
			...
			if (uindexes.Where(i => i >= _filesCount).Any(
				i => !ThrowException(null, new ArgumentOutOfRangeException("indexes", "Index must be less than " +
					_filesCount.Value.ToString(CultureInfo.InvariantCulture) + "!"))))
			{
				return;
			}
#else
			foreach (uint i in uindexes)
			{
				if (i >= _filesCount)
				{
					if (!ThrowException(null, new ArgumentOutOfRangeException("indexes", "Index must be less than " +
						_filesCount.Value.ToString(CultureInfo.InvariantCulture) + "!")))
					{
						return;
					}
		...
		public void ExtractFiles(string directory, params string[] fileNames)
		{
			...
			foreach (string fn in fileNames)
			{
				...
				{
					if (
						!ThrowException(null, new ArgumentOutOfRangeException("fileNames",
							"File \"" + fn + "\" was not found in the archive file table.")))
					{
						return;
					}
				}
